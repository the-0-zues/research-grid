"""
dgsweep_build_8pmu.py
=====================
8-PMU variant. Adds PMU9 and PMU17 to the 6-PMU set
(PMU2, PMU4, PMU5, PMU8, PMU9, PMU11, PMU17, PMU18 -> 8 PMUs, 31 feats each
= 248 feats).

Topology (see build_geometry):
  * PMU5 splits PMU4->PMU8 at km 1.044  (as in the 6-PMU build).
  * PMU9 sits ON the PMU2->PMU11 trunk at km 2.385, between PMU8 (1.907) and
    PMU11 (2.719). It therefore SPLITS the PMU8->PMU11 section into PMU8->PMU9
    and PMU9->PMU11, giving a 5-section bounding-pair distance chain.
  * PMU17 and PMU18 are OFF the trunk (branch taps in the PMU5..PMU8 region,
    attach km 1.843). They add observability features only.

Pure feature rebuild from the DGsweep raws (all 19 PMUs already captured) --
no re-simulation. Same no-DG CT noise (module B), fuse-driven binary smart
meters, and dropout sweep as the 4-/6-PMU builds.

Layout:
  PMU_dataset_v2/DGsweep_8PMU/{n}DG/{Faulty,Clean}/{010,020,050,080,100}pct/results.csv
...
# ---- promote module B to 8 PMUs (PMU5 splits PMU4->PMU8, PMU9 splits PMU8->PMU11) ----
B.TARGET = ["PMU2", "PMU4", "PMU5", "PMU8", "PMU9", "PMU11", "PMU17", "PMU18"]
B.PAIRS  = [("PMU2", "PMU4"), ("PMU4", "PMU5"), ("PMU5", "PMU8"),
            ("PMU8", "PMU9"), ("PMU9", "PMU11")]

BASE    = r"C:\EPRI_Ckt7_DER_Project\Realistic_Simulation\PMU_dataset_v2\DGsweep_8PMU"
...
DGRAW   = r"C:\EPRI_Ckt7_DER_Project\Realistic_Simulation\PMU_dataset_v2\DGsweep"
...
S58  = KM_AT[BUS1["PMU8"]]
S89  = KM_AT[BUS1["PMU9"]]
S911 = KM_AT[BUS1["PMU11"]]
print(f"chain sections: PMU2->PMU4 [0.000,{S24:.3f}]  PMU4->PMU5 [{S24:.3f},{S45:.3f}]  "
      f"PMU5->PMU8 [{S45:.3f},{S58:.3f}]  PMU8->PMU9 [{S58:.3f},{S89:.3f}]  "
      f"PMU9->PMU11 [{S89:.3f},{S911:.3f}] km")


def prep(branch, n_dg):
    d = os.path.join(DGRAW, f"{n_dg}DG", branch)
    ...
    if branch == "Faulty":
        ...
        bounds = {("PMU2","PMU4"): (0.0, S24), ("PMU4","PMU5"): (S24, S45),
                  ("PMU5","PMU8"): (S45, S58), ("PMU8","PMU9"): (S58, S89),
                  ("PMU9","PMU11"): (S89, S911)}
