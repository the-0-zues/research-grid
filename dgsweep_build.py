...
dgsweep_build.py
================
Assemble the DG-sweep datasets from the raws. For each DG count (0/1/2/3) and
branch (Faulty/Clean): noise -> 4-PMU features (currents + voltage magnitudes
+ angle deltas + dV0_ang, 31/PMU) -> bounding-pair distance estimate +
true_dist/lateral_run/true_path labels -> fuse-driven binary smart meters.
The dropout levels are then cheap re-slices of the same SM base.

Layout:
  PMU_dataset_v2/DGsweep/{n}DG/{Faulty,Clean}/{010,020,050,080,100}pct/results.csv
...
BASE    = r"C:\EPRI_Ckt7_DER_Project\Realistic_Simulation\PMU_dataset_v2\DGsweep"
...
S24, S48, S811 = KM_AT[BUS1["PMU4"]], KM_AT[BUS1["PMU8"]], KM_AT[BUS1["PMU11"]]


def prep(branch, n_dg):
    d = os.path.join(BASE, f"{n_dg}DG", branch)
    ...
    if branch == "Faulty":
        ...
        bounds = {("PMU2","PMU4"):(0.0,S24), ("PMU4","PMU8"):(S24,S48), ("PMU8","PMU11"):(S48,S811)}
